module OscService
  class Bundle < Array
    ...
    def initialize(timetag = nil, *args)
      args.each { |arg| raise TypeError, "#{arg.inspect} is required to be a Bundle or Message" unless arg.is_a?(Bundle) || arg.is_a?(Message) }
      raise TypeError, "#{timetag.inspect} is required to be Time or nil" unless timetag.nil? || timetag.is_a?(Time)
